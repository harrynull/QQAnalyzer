...
class messages:
    ...
    def __init__(self, file_content, buffer,buffer2, buffer_valid):
        ...
        # init
        name_re=re.compile(r"^(\d{4}-\d{2}-\d{2} \d{1,2}:\d{2}:\d{2}) (.*)\((.*?)\)\n")
        name_email=re.compile(r"^(\d{4}-\d{2}-\d{2} \d{1,2}:\d{2}:\d{2}) (.*)\<(.*?)\>\n")

        # get the group name in the 5th line
        self.name=(lambda line:line[line.index(":")+1:-1])(file_content[GROUP_NAME_LINE])

        print("Start analyzing " + self.name + "...")
...
parser.add_argument("mode", help="mode you want to use",choices =("user","all","week","hour"), default="")
...
def analyze_week(msgs, qq):
    ...
    for m in msgs.getMessages():
        if m.qq != qq and qq!="": continue
        key=pd.Timestamp(m.time, unit='s').day_name()
        stat[key]=stat.get(key, 0)+1
    return stat
    
# to analyze the relationship between the numbers of messages and each hour of a day
def analyze_hr(msgs, qq):
    stat={}
    for m in msgs.getMessages():
        if m.qq != qq and qq!="": continue
        key=pd.Timestamp(m.time, unit='s').hour
        stat[key]=stat.get(key, 0)+1
    return stat
...
output=open(file_name+"_"+args.mode+".csv","w",encoding="utf_8_sig" if args.use_bom else "utf8")
...
"hour": lambda:print_plain(output,analyze_hr(msgs,args.qq))
}
